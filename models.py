...
class Asset(db.Model):
    ...
    warranty_period = db.Column(db.Integer)  # 保修期限，单位月
    location = db.Column(db.String(128))
    department_id = db.Column(db.Integer, db.ForeignKey('department.id'))
    status = db.Column(db.String(20), default='in_stock')  # in_stock, in_use, maintenance, disposed
    description = db.Column(db.Text)
    qr_code = db.Column(db.String(256))  # 二维码路径或内容
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class AssetBorrow(db.Model):
    ...
    borrow_type = db.Column(db.String(20), default='borrow')  # borrow, use
    purpose = db.Column(db.String(256))
    expected_return_date = db.Column(db.DateTime)
    actual_return_date = db.Column(db.DateTime)
    status = db.Column(db.String(20), default='pending')  # pending, approved, borrowed, returned, rejected
...
class AssetDisposal(db.Model):
    ...
    disposal_type = db.Column(db.String(20))  # scrap, sell, donate, lost
...
class Budget(db.Model):
    ...
    warning_threshold = db.Column(db.Float, default=80.0)  # 百分比
...
class BudgetUsageLog(db.Model):
    ...
    usage_type = db.Column(db.String(20))  # purchase, adjustment
...
class Inventory(db.Model):
    ...
    name = db.Column(db.String(128))
    inventory_type = db.Column(db.String(20), default='annual')  # annual, quarterly, temporary
    operator_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    department_id = db.Column(db.Integer, db.ForeignKey('department.id'))
...
class InventoryItem(db.Model):
    ...
    inventory_result = db.Column(db.String(20))  # normal, profit, loss, discrepancy
...
class Maintenance(db.Model):
    ...
    asset_id = db.Column(db.Integer, db.ForeignKey('asset.id'))
    operator_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    maintenance_type = db.Column(db.String(20))  # repair, maintenance, inspection
    description = db.Column(db.Text)
    schedule_date = db.Column(db.DateTime)
    actual_date = db.Column(db.DateTime)
    duration = db.Column(db.Float, default=0.0)  # 运行时长，单位小时
...
class MaintenancePlan(db.Model):
    ...
    asset_id = db.Column(db.Integer, db.ForeignKey('asset.id'))
    plan_type = db.Column(db.String(20))  # category, specific
    maintenance_type = db.Column(db.String(20))  # repair, maintenance, inspection
    interval_days = db.Column(db.Integer)  # 间隔天数
    next_maintenance_date = db.Column(db.DateTime)
    duration = db.Column(db.Float, default=0.0)  # 预计运行时长
    description = db.Column(db.Text)
...
class Approval(db.Model):
    ...
    approval_type = db.Column(db.String(20))  # purchase, borrow, disposal, transfer
    purchase_request_id = db.Column(db.Integer, db.ForeignKey('purchase_request.id'))
    ...
    level = db.Column(db.Integer, default=1)  # 审批级别
...
class Notification(db.Model):
    ...
    notification_type = db.Column(db.String(20))  # maintenance_reminder, borrow_return, approval, budget_warning
    related_type = db.Column(db.String(20))  # asset, purchase_request, approval, etc.
    related_id = db.Column(db.Integer)
    is_read = db.Column(db.Boolean, default=False)
    send_method = db.Column(db.String(20), default='system')  # system, wechat, sms, email
